...
//
// ANSWER:
// This allows for the properties not to be modified by
// outside packages. This reduces the chances of bugs
// happening or being created from outside packages and
// allows for debugging to be consolidated into one location.
type ToDo struct {
	toDoMap    DbMap
	dbFileName string
}
...
func (t *ToDo) RestoreDB() error {
	...
	backUpFile, err := os.Open(backupFileName) // Open backup file
	if err != nil {                            //Return error if backup file does not exist
		return err
	}
	defer backUpFile.Close()

	mainFileLocation, err := os.Create(dbFileName) // Create or open the destination file
	if err != nil {                                //Return error if db file does not exist
		return err
	}
	defer mainFileLocation.Close()

	_, err = io.Copy(mainFileLocation, backUpFile) //copy data from main file destination to backup file
	if err != nil {                                //Return error if an issue copying
		return err
	}
...
func (t *ToDo) AddItem(item ToDoItem) error {
	...
	err = t.saveDB()
	if err != nil { //Return error if issue saving
		return err
	}
