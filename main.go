...
//
//	 YOUR ANSWER:
//				We are creating pointer flags of different types for different purposes
//				First paramater is the pointer to where the data is being stored.
//				Second is name of the flag. Third is the default value.
//				Fourth just an explanation of the flag. The flags are associated to AppOptType
//				iota to detirmine what functionality will be performed. It will do some
//				error handling in cases where flag combinations are not allowed or not implemeneted
...
func main() {
	...
	case CHANGE_ITEM_STATUS:
		...
		if err != nil {
			fmt.Println("Change status option requires a valid JSON todo item string")
			break
		}
		fmt.Println("Ok")
	default:
