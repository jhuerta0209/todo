...
func TestAddHardCodedItem(t *testing.T) {
	...
	assert.NoError(t, err, "Error looking up item in DB")

	assert.Equal(t, item, addedItem, "Item in DB does not match the added item")
}
...
func TestJuan(t *testing.T) {
	item := db.ToDoItem{
		Id:     10,
		Title:  "This is Juan's test case",
		IsDone: false,
	}
	err := DB.AddItem(item)
	assert.NoError(t, err, "Error adding item to DB")
	...
	assert.NoError(t, err, "Error updating stauts")

	item.IsDone = true
	storedItem, err := DB.GetItem(1)
	assert.Equal(t, item, storedItem, err)
}
